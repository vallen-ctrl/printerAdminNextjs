...
interface callbackValue{
  callbackValue: (dat: {name:string, email: string, password:string}) => void,
  issignUp:boolean,
  redirectURL: string 
}

const Form = ({callbackValue, issignUp, redirectURL}:callbackValue) => {
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");

  const handler = (e: React.FormEvent) => {e.preventDefault(); callbackValue({name: name, email:email, password:password})}
