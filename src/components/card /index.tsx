...
const Card = ({id,name, desc, price, onClick, star, pembeli}:card) => {


  return (
    <StyledWrapper>
      <div className="card" onClick={onClick} data-id={id}>
        <div className="card-img"><div className="img" /></div>
        <div className="card-title">{name}</div>
        <div className="card-subtitle">
          <Collapse collapsedSize={20}>{desc}</Collapse>
        </div>
        <hr className="card-divider" />
        <div className="card-footer">
          <div className="card-price"><span>Rp </span>{price}</div>
          <div className=" text-black text-[15px]">{star}/5 <Star style={{width:20 }}></Star></div>
        </div>
        <div className="
